...
module Brakeman
  ...
  def self.run options
    ...
    if !File.expand_path(options[:app_path]).include? "/rails2"
        base_dir = File.dirname(options[:app_path])
        rails_location = "#{base_dir}/rails2/"
        helper_location = "#{base_dir}/rails2/app/helpers/"
        # Create the helpers folder if they doesn't exist yet.
        unless File.directory?(helper_location)
            FileUtils.mkdir_p(helper_location)
            notify "Create folder: #{helper_location}"
        end
        if File.directory?(options[:app_path])
            # Copy the necessary files to the rails application
            notify "Current location: #{options[:app_path]}"
            notify "Current contents: #{Dir.entries(options[:app_path])}"
            appended_path = File.join(options[:app_path], ".")
            notify "Current location: #{appended_path}"

            FileUtils.cp_r appended_path, helper_location

            notify "New location: #{rails_location}"
            notify "New contents: #{Dir.entries(rails_location)}"

            options[:app_path] = rails_location
        else
            notify "Current location: #{options[:app_path]}"
            notify "Current contents: #{options[:app_path]}"
            appended_path = options[:app_path]
