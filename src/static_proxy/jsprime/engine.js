...
function analyze(input, source_sink_list, callback) {
    var analyze_results;
    source_sink_list = JSON.parse(source_sink_list)
    anon_func_names = [];
    options = {
        loc: true,
        comment: false,
    ...
    if ((source.length == 0) || (sink.length == 0)) {
        for (var i in source_sink_list) {
            if (source_sink_list[i].array[12] == 1) {
                source.push(source_sink_list[i].array[2])
            } else if (source_sink_list[i].array[12] == 2) {
                sink.push(source_sink_list[i].array[2])
            } else {
                sink.push(source_sink_list[i].array[2])
            }
        }
	...
	var analyzer = require('./analyzer.js')(real_variable_const, real_variable_var, convertedFunction, source, sink, code, function(analyze_results) {

		var split_lines = code.split('\n')
		analyze_results.sources_sinks = raw_sources_sinks

		for (var j = 0; j < raw_sources_sinks.length; j++) {
    			var function_name = ""
   
    			// Iterate through lines in reverse
    			for (var i = raw_sources_sinks[j].loc.start.line; i > 0; i--) {
				
				// Check for functions
				if (split_lines[i].indexOf("function") != -1) {
					
	    				// Ignore calback functions
	    				if ((split_lines[i].indexOf("function(") == -1) && (split_lines[i].indexOf("function (") == -1)) {
						parsed_line = esprima.parse(split_lines[i] + "}")
						raw_sources_sinks[j].calling_function = parsed_line.body[0].id.name
						break
	    				}
				}
    			}
		...
		process.send(JSON.stringify(analyze_results))

	})
    })
    //callback(analyze_results)
}

function traverse(node, func) {
    if (node.type) {
	    if (node.type == "MemberExpression") {
		if (source.includes(node.object.name + "." + node.property.name)) {
			node.source_sink = "SOURCE"
			raw_sources_sinks.push(node)
		}
		if (sink.includes(node.object.name + "." + node.property.name)) {
			node.source_sink = "SINK"
			raw_sources_sinks.push(node)
		}
	    }
    }
...
function getVariables(node) {
    ...
    if (node.type == "AssignmentExpression" && node.left && node.right) {
        ...
        var varObj = [];
        var lineObj = [];
        if (node.right.value) varVal = node.right.value + "#type#const";
        else if (node.right.name) varVal = node.right.name + "#type#var";
        else {
            var varVal = "";
            var node2;
            node2 = node.right;
            ...
            while (binaryNode.type == "BinaryExpression") {
                if (binaryNode.left.name != undefined) varVal = varVal + "+" + binaryNode.left.name;
                if (binaryNode.left.value != undefined && binaryNode.right.name != undefined) varVal = varVal + "+" + binaryNode.right.name;
                else if (binaryNode.right.name != undefined && binaryNode.left != undefined) varVal = varVal + "+" + binaryNode.right.name;
                var varName2 = "";
                var nodeValue = "";
                var binaryNode2 = null;
                if (binaryNode.right.type == "MemberExpression") binaryNode2 = binaryNode.right;
                else binaryNode2 = binaryNode.left;
                while (binaryNode2.type == "MemberExpression") {
                    if (binaryNode2.property) {
                        var nodeValue = "";
                        if (binaryNode2.property.name) nodeValue = binaryNode2.property.name
                        else if (binaryNode2.property.value) nodeValue = binaryNode2.property.value
                        if (varName2 == "") varName2 = nodeValue + varName2;
                        else varName2 = nodeValue + "." + varName2;
                    }
            ...
            if (node2.callee) {
                ...
                for (var j = 0; j < node2.arguments.length; j++) {
                    ...
                    if (node2.arguments[j].name) data.arguments.variables.push(node2.arguments[j].name);
                    ...
                    while (node4.type == "BinaryExpression") {
                        if (node4.left.name != undefined) multipleArgs = node4.left.name + multipleArgs;
                        else if (node4.left.value != undefined && node4.right.name != undefined) multipleArgs = node4.right.name + multipleArgs;
                        else if (node4.right.name != undefined && node4.left != undefined) multipleArgs = node4.operator + node4.right.name + multipleArgs;
                        node4 = node4.left;
                    }
                    if (multipleArgs != "") data.arguments.variables.push(multipleArgs);
                }
                node2 = node2.callee;
            ...
            while (node2.type == "MemberExpression") {
                if (node2.property) {
                    var nodeValue = "";
                    if (node2.property.name) nodeValue = node2.property.name;
                    else if (node2.property.value) nodeValue = node2.property.value;
                    if (varVal == "" && discardFirstParam == true) {
                        nodeValue = "";
                        discardFirstParam = false;
                    }
                    if (varVal == "") varVal = nodeValue + varVal;
                    else varVal = nodeValue + "." + varVal;
                }
                if (node2.object.name) {
                    if (varVal != "") varVal = node2.object.name + "." + varVal;
                    else varVal = node2.object.name;
                ...
                for (i = 0; i < node2.properties.length; i++) {
                    if (node2.properties[i].type === 'Property') {
                        if (node2.properties[i].key.name) {
                            ...
                            if (node2.properties[i].value.value != undefined) varObj.push(node2.properties[i].key.name + "#prop#" + node2.properties[i].value.value + "#type#const");
                            else if (node2.properties[i].value.name != undefined) varObj.push(node2.properties[i].key.name + "#prop#" + node2.properties[i].value.name + "#type#var");
                            else {
                                ...
                                while (node4.type == "MemberExpression") {
                                    if (node4.property) {
                                        var nodeValue = "";
                                        if (node4.property.name) nodeValue = node4.property.name
                                        else if (node4.property.value) nodeValue = node4.property.value
                                        if (varName2 == "") varName2 = nodeValue + varName2;
                                        else varName2 = nodeValue + "." + varName2;
                                    }
                                ...
                                if ((node4.type == "FunctionExpression") || (node4.type == "ArrowFunctionExpression")) {
                                        ...
                                        for (var b1 = 0; b1 < node4.body.body.length; b1++) {
                                            if (node4.body.body[b1].type = "ReturnStatement" && node4.body.body[b1].argument) {
                                                ...
                                                while (bNode.type == "MemberExpression") {
                                                    if (bNode.property.name) nodeValue = bNode.property.name
                                                    else if (bNode.property.value) nodeValue = bNode.property.value
                                                    if (funcName4 == "") funcName4 = nodeValue + funcName4;
                                                    else funcName4 = nodeValue + "." + funcName4;
        ...
        if (node.left.name) {
            ...
            while (node.type == "MemberExpression") {
                if (node.property) {
                    var nodeValue = "";
                    if (node.property.name) nodeValue = node.property.name
                    else if (node.property.value) nodeValue = node.property.value
                    if (varName == "") varName = nodeValue + varName;
                    else varName = nodeValue + "." + varName;
                }
                if (node.object.name) {
                    var nodeValue = "";
                    if (node.object.name) nodeValue = node.object.name
                    else if (node.object.value) nodeValue = node.object.value
        ...
        var lineObj = [];
        var lineNumber = 0;
        if (node.init.value) varVal = node.init.value + "#type#const";
        else if (node.init.name) varVal = node.init.name + "#type#var";
        else {
            var varVal = "";
            var node2;
            node2 = node.init;
            ...
            while (binaryNode.type == "BinaryExpression") {
                if (binaryNode.left.name != undefined) varVal = varVal + "+" + binaryNode.left.name;
                if (binaryNode.left.value != undefined && binaryNode.right.name != undefined) varVal = varVal + "+" + binaryNode.right.name;
                if (binaryNode.right.name != undefined && binaryNode.left != undefined) varVal = varVal + "+" + binaryNode.right.name;
                var varName2 = "";
                var nodeValue = "";
                var binaryNode2 = null;
                if (binaryNode.right.type == "MemberExpression") binaryNode2 = binaryNode.right;
            ...
            while (node2.type == "MemberExpression") {
                if (node2.property) {
                    ...
                    else if (node2.callee && node2.callee.name) nodeValue = node2.callee.name;
                    if (varVal == "" && discardFirstParam == true) {
                        nodeValue = "";
                        discardFirstParam = false;
                    }
                ...
                if (node2.object.name) {
                    ...
                    for (j = 0; j < node2.object.arguments.length; j++) {
                        if (node2.object.arguments[j].value) funcArgs = funcArgs + "#args#" + node2.object.arguments[j].value;
                        else if (node2.object.arguments[j].name) funcArgs = funcArgs + "#args#" + node2.object.arguments[j].name;
                    }
                }
                if (node2.object.callee) {
                    var node5 = node2;
...
function getFunctions(node) {
	...
	var data = {
        ...
        for (i = 0; i < node.params.length; i++) {
            if (node.params[i].name) data.arguments.variables.push(node.params[i].name);
        ...
        traverseMini(body, function(node2) {
            ...
            if (node2.type == "ReturnStatement" && node2.argument) {
                var args = "";
                if (node2.argument.name) data.returns.variables.push(node2.argument.name);
                ...
                if (node2.argument.type == "CallExpression") {
                    ...
                    var funcName4 = "";
                    var nodeValue = "";
                    if (node4.name) funcName4 = node4.name;
                    while (node4.type == "MemberExpression") {
                        if (node4.property.name) nodeValue = node4.property.name
                        else if (node4.property.value) nodeValue = node4.property.value
                        if (funcName4 == "") funcName4 = nodeValue + funcName4;
                    ...
                    for (var j = 0; j < node2.argument.arguments.length; j++) {
                        if (node2.argument.arguments[j].name) data.returns.variables.push(node2.argument.arguments[j].name);
                        ...
                        while (node4.type == "MemberExpression") {
                            ...
                            if (objName4 == "") objName4 = nodeValue + objName4;
                            else objName4 = nodeValue + "." + objName4;
                            if (node4.object.name) {
                ...
                while (bNode.type == "MemberExpression") {
                    ...
                    if (bReturnValue == "") bReturnValue = nodeValue + bReturnValue;
                    else bReturnValue = nodeValue + "." + bReturnValue;
                    if (bNode.object.name) {
       ...
       var data = {
        ...
        if ((node.callee.type == "FunctionExpression" || node.callee.type == "ArrowFunctionExpression") && !node.callee.id) {
            data.name = "#NONAME#" + makeRandomName();
            delete data['startScope'];
            delete data['endScope'];
            var node2 = node.callee;
            for (i = 0; i < node2.params.length; i++) {
                if (node2.params[i].name) data.arguments.variables.push(node2.params[i].name);
        ...
        for (i = 0; i < node.arguments.length; i++) {
            ...
            if (node.arguments[i].name) data.arguments.variables.push(node.arguments[i].name);
            ...
            if (node5.type == "CallExpression") {
                ...
                if (node5.name) {
                ...
                } else if (node5.type == "FunctionExpression" || node5.type == "ArrowFunctionExpression") {
                    if (node5.id) {
                    ...
                    for (a1 = 0; a1 < node5.params.length; a1++) {
                        if (node5.params[a1].name) data2.arguments.variables.push(node5.params[a1].name);
                ...
                while (node5.type == "MemberExpression") {
                    if (node5.property.name) nodeValue = node5.property.name
                    else if (node5.property.value) nodeValue = node5.property.value
                    if (funcName4 == "") funcName4 = nodeValue + funcName4;
                    else funcName4 = nodeValue + "." + funcName4;
            ...
            if (funcName4 != "") data.arguments.variables.push(funcName4);
            while (node2.type == "BinaryExpression") {
                if (node2.left.name != undefined) data.arguments.variables.push(node2.left.name);
                if (node2.right.name != undefined) data.arguments.variables.push(node2.right.name);
                var node6 = node2;
                while (node6.type == "BinaryExpression") {
                    ...
                    if (node6.left) {
                        ...
                        if (funcName4 != "" && data.arguments.variables.indexOf(funcName4) == -1) data.arguments.variables.push(funcName4);
                    }
                    node5 = node6;
                    if (node6.right) {
                        var nodeValue = "";
    ...
    if (node.type === 'VariableDeclarator' && node.init) {
	    ...
	    for (i = 0; i < node.init.properties.length; i++) {
                if (node.init.properties[i].type === 'Property') {
                    if (node.init.properties[i].key.name && (node.init.properties[i].value.type == "FunctionExpression" || node.init.properties[i].value.type == "ArrowFunctionExpression")) {
                        var funcName = node.id.name + "." + node.init.properties[i].key.name;
                        for (j = 0; j < node.init.properties[i].value.params.length; j++) {
                            if (node.init.properties[i].value.params[j].name) data.arguments.variables.push(node.init.properties[i].value.params[j].name);
            ...
            if (node.id.name) {
                ...
                for (var j = 0; j < node.init.params.length; j++) {
                    if (node.init.params[j].name) data.arguments.variables.push(node.init.params[j].name);
                ...
                traverseMini(body, function(node2) {
                    if (node2.type == "ReturnStatement" && node2.argument != undefined) {
                        var args = "";
                        if (node2.argument.name != undefined) data.returns.variables.push(node2.argument.name);
                        ...
                        if (node2.argument.type == "CallExpression") {
                            ...
                            var argValue = "";
                            for (var j = 0; j < node2.argument.arguments.length; j++) {
                                if (node2.argument.arguments[j].name) data.arguments.variables.push(node2.argument.arguments[j].name);
                                if (node2.argument.arguments[j].value) {
                            ...
                            if (funcName4 != "") data.returns.variables.push(funcName4);
                        }
                    }
                });
                real_func_names.push(data);
	...
	var data = {
        ...
        if (node.right.type == "ObjectExpression" && node.right.properties) {
            for (i = 0; i < node.right.properties.length; i++) {
                if (node.right.properties[i].type === 'Property') {
                    if (node.right.properties[i].key.name && (node.right.properties[i].value.type == "FunctionExpression" || node.right.properties[i].value.type == "ArrowFunctionExpression")) {
		...
		if (node.left.name) {
                ...
                for (var j = 0; j < node.right.params.length; j++) {
                    if (node.right.params[j].name) data.arguments.variables.push(node.right.params[j].name);
                ...
                var body = node.right.body;
                node = node.left;
                if (node.left) data.line = node.left.loc.start.line;
                while (node.type == "MemberExpression") {
                    if (node.property) {
                        if (funcName == "") funcName = node.property.name + funcName;
                        else funcName = node.property.name + "." + funcName;
                    }
                    if (node.object.name) {
