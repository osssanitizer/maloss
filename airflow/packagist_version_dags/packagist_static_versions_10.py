...
dag = DAG('packagist_static_versions_10', default_args=default_args, schedule_interval=None)
# periodically run the dag
# dag = DAG('tutorial', default_args=default_args, schedule_interval=timedelta(days=1))

# load dep_tree for packages, relative to AIRFLOW_HOME
packagist_dep_path = "./dags/packagist.with_stats.popular.versions.dep_graph_10.pickle"
dep_tree = pickle.load(open(packagist_dep_path, "rb"))
...
def invalid_dependency(pkg_name, pkg_version):
    return pkg_version.startswith(('http://', 'https://')) or '/' in pkg_version or '+' in pkg_version


def get_bash_op(pkg_name, pkg_version, dag, configpath='/home/maloss/config/astgen_php_smt.config', cache_dir='/home/maloss/metadata', outdir='/home/maloss/result'):
    return BashOperator(
        task_id='%s..%s' % (get_sanitized_pkgname(pkg_name=pkg_name), pkg_version),
        execution_timeout=timedelta(hours=2),
        bash_command='cd /home/maloss/src/ && python main.py astfilter -n %s -v %s -c %s -d %s -o %s -l php' % (pkg_name, pkg_version, configpath, cache_dir, outdir),
        dag=dag)
