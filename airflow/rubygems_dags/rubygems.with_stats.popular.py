...
dag = DAG('rubygems.with_stats.popular', default_args=default_args, schedule_interval=None)
# periodically run the dag
# dag = DAG('tutorial', default_args=default_args, schedule_interval=timedelta(days=1))

# load dep_tree for packages, relative to AIRFLOW_HOME
rubygems_dep_path = "./dags/rubygems.with_stats.popular.pickle"
dep_tree = pickle.load(open(rubygems_dep_path, "rb"))
logging.info("loaded dep_tree with %d nodes", dep_tree.number_of_nodes())


def get_bash_op(pkg_name, dag, configpath='/home/maloss/config/astgen_ruby_smt.config', cache_dir='/home/maloss/metadata', outdir='/home/maloss/result'):
    return BashOperator(
        task_id=pkg_name,
        execution_timeout=timedelta(hours=2),
        bash_command='cd /home/maloss/src/ && python main.py astfilter --ignore_dep_version -n %s -c %s -d %s -o %s -l ruby' % (pkg_name, configpath, cache_dir, outdir),
        dag=dag)
