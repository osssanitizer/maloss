...
dag = DAG('rubygems_static_versions_10', default_args=default_args, schedule_interval=None)
# periodically run the dag
# dag = DAG('tutorial', default_args=default_args, schedule_interval=timedelta(days=1))

# load dep_tree for packages, relative to AIRFLOW_HOME
rubygems_dep_path = "./dags/rubygems.with_stats.popular.versions.dep_graph_10.pickle"
dep_tree = pickle.load(open(rubygems_dep_path, "rb"))
logging.info("loaded dep_tree with %d nodes", dep_tree.number_of_nodes())


def get_bash_op(pkg_name, pkg_version, dag, configpath='/home/maloss/config/astgen_ruby_smt.config', cache_dir='/home/maloss/metadata', outdir='/home/maloss/result'):
    return BashOperator(
        task_id='%s..%s' % (pkg_name, pkg_version),
        execution_timeout=timedelta(hours=2),
        bash_command='cd /home/maloss/src/ && python main.py astfilter -n %s -v %s -c %s -d %s -o %s -l ruby' % (pkg_name, pkg_version, configpath, cache_dir, outdir),
        dag=dag)
