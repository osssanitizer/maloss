...
dag = DAG('maven_static_19', default_args=default_args, schedule_interval=None)
# periodically run the dag
# dag = DAG('tutorial', default_args=default_args, schedule_interval=timedelta(days=1))

# load dep_tree for packages, relative to AIRFLOW_HOME
maven_dep_path = "./dags/maven.dep_graph_19.pickle"
...
def get_bash_op(pkg_name, dag, configpath='/home/maloss/config/astgen_java_smt.config', cache_dir='/home/maloss/metadata', outdir='/home/maloss/result'):
    return BashOperator(
        task_id=get_sanitized_pkgname(pkg_name=pkg_name),
        execution_timeout=timedelta(hours=2),
        bash_command='cd /home/maloss/src/ && python main.py astfilter -n %s -c %s -d %s -o %s -l java' % (pkg_name, configpath, cache_dir, outdir),
        dag=dag)
