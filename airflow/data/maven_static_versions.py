...
dag = DAG('maven_static_versions', default_args=default_args, schedule_interval=None)
# periodically run the dag
# dag = DAG('tutorial', default_args=default_args, schedule_interval=timedelta(days=1))

# load dep_tree for packages, relative to AIRFLOW_HOME
maven_dep_path = "./dags/maven.popular.versions.dep_graph.pickle"
dep_tree = pickle.load(open(maven_dep_path, "rb"))
...
def get_bash_op(pkg_name, pkg_version, dag, configpath='/home/maloss/config/astgen_java_smt.config', cache_dir='/home/maloss/metadata', outdir='/home/maloss/result'):
    return BashOperator(
        task_id='%s..%s' % (get_sanitized_pkgname(pkg_name=pkg_name), pkg_version),
        execution_timeout=timedelta(hours=2),
        bash_command='cd /home/maloss/src/ && python main.py astfilter -n %s -v %s -c %s -d %s -o %s -l java' % (pkg_name, pkg_version, configpath, cache_dir, outdir),
        dag=dag)
